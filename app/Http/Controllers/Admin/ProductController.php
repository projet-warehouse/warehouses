...
// --------------------- add product ------------------ 
    ...
    public function storeProduct(Request $request){
        ...
        Image::make($imag_one)->resize(270,270)->save('images/produits/'.$name_gen);       
        $img_url1 = 'images/produits/'.$name_gen;

        $imag_two = $request->file('image_two');                
        $name_gen = hexdec(uniqid()).'.'.$imag_two->getClientOriginalExtension();
        Image::make($imag_two)->resize(270,270)->save('images/produits/'.$name_gen);       
        $img_url2 = 'images/produits/'.$name_gen;

        $imag_three = $request->file('image_three');                
        $name_gen = hexdec(uniqid()).'.'.$imag_three->getClientOriginalExtension();
        Image::make($imag_three)->resize(270,270)->save('images/produits/'.$name_gen);       
        $img_url3 = 'images/produits/'.$name_gen;
        ...
        return Redirect()->back()->with('success','Produit ajouté avec sucess');
    ...
    public function updateProduct(Request $request){
        ...
        return Redirect()->route('manage-products')->with('success','Modifiaction produit avec succès');
    }
    ...
    public function updateImage(Request $request){
        ...
        if ($request->has('image_one') && $request->has('image_two')) {
            ...
            $imag_one = $request->file('image_one');                
             ...
             return Redirect()->route('manage-products')->with('success','image modifiée avec succès');
         }
        ...
        if ($request->has('image_two')) {
         ...
         if ($request->has('image_three')) {
            ...
            $imag_one = $request->file('image_three');                
             $name_gen = hexdec(uniqid()).'.'.$imag_one->getClientOriginalExtension();
             Image::make($imag_one)->resize(270,270)->save('image modifiée avec succès'.$name_gen);       
             $img_url1 = 'images/produits/'.$name_gen;
    ...
    public function destroy($product_id){
        ...
        return Redirect()->back()->with('delete','Supprimée avec succès');
    }

     // status inactive 
     public function Inactive($product_id){
        Product::findOrFail($product_id)->update(['status' => 0]);
        return Redirect()->back()->with('status','Produit inactive');
    }
    ...
    public function Active($product_id){
        ...
        return Redirect()->back()->with('status','Produit Activé');
    }
}
